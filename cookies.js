...
http.request(login, function onRes(res) {
    ...
    request(options, function(error, response, body) {
        ...
        fs.appendFile('searches.csv', linesExceptFirst, function(err, result) { //Put our report minus the first two lines into a csv file
            if (err) {
                console.error(err)
            }
            console.log('file written, now lets parse it')
            lr = new LineByLineReader('searches.csv');
